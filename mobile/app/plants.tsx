...
export default function PlantsScreen() {
  ...
  useEffect(() => {
    try {
      ...
      setResult(analysePlants(data, period, regionKey));
  ...
  if (!result) return <LoadingState message="Checking plant watering conditions…" />;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>💧 Should I water the plants?</Text>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.adviceCard}>
          <Text style={styles.adviceTitle}>Watering advice</Text>
          <Text style={styles.adviceText}>🧔 Pots, baskets and newly planted plants dry faster. Always check those first.</Text>
          <Text style={styles.adviceText}>🌱 Garden beds retain moisture longer — check soil before watering.</Text>
          <Text style={styles.adviceText}>💧 Water in the morning or evening to reduce evaporation.</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  adviceCard: { backgroundColor: Colors.white, borderRadius: Radius.lg, padding: Spacing.lg, borderWidth: 1, borderColor: Colors.line, gap: Spacing.md, ...Shadow.card },
  adviceTitle: { fontSize: Typography.h4, fontWeight: '700', color: Colors.navy },
  adviceText: { fontSize: Typography.small, color: Colors.muted, lineHeight: 20 },
